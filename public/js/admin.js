...
$(function(){ 
    ...
    $("body").on('DOMNodeInserted', function(e) {
        if($(e.target).find(".owl-carousel").length > 0){
            $(e.target).find(".owl-carousel").owlCarousel({
                items:1,
                loop:false,
                autoplay:false,
                nav: true,
...
$(document).ready(function(){
    $("input.live, textarea.live, select.live, .trumbowyg-editor").on("change keyup", function(e){
        ...
        if(e.type == 'change'){
            ...
            if($(this).hasClass("trumbowyg-editor")){
                var editor = this;
...
function removeOption(optionID){
    $.post('?cmd=removeOption', {optionID : optionID}, function(data){
        location.reload();
    });
}
...
function removeProducts(productID){
    if(confirm("Ցանկանում եք հեռացնել Ապրանքը") == true) { 
        $.post('?cmd=removeProducts', {productID : productID}, function(data){
            location.reload();
        });
    }
}
...
function removeCareer(id){
    if(confirm("Ցանկանում եք հեռացնել կարիերան") == true) {
        $.post('?cmd=removeCareer', {id : id}, function(data){
            location.reload();
        });
    }
}
...
function removeEvents(id){
    if(confirm("Ցանկանում եք հեռացնել միջոցառումը") == true) {
        $.post('?cmd=removeEvents', {id : id}, function(data){
            location.reload();
        });
    }
}
...
function removePartner(id){
    if(confirm("Ցանկանում եք հեռացնել գործընկերոջը") == true) {
        $.post('?cmd=removePartner', {id : id}, function(data){
            location.reload();
        });
    }
}
...
function removeConsulting(id){
    if(confirm("Ցանկանում եք հեռացնել խորհրդատվությունը") == true) {
        $.post('?cmd=removeConsulting', {id : id}, function(data){
            location.reload();
        });
    }
}